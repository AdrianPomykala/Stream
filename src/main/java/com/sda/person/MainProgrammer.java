...
public class MainProgrammer {
    public static void main(String[] args) {
        ...
        List<Programmer> mezczyzni = programmers.stream()
                .filter((p) -> {
                    return p.getPerson().isMale();
        }).collect(Collectors.toList());
        System.out.println(mezczyzni);
        List<Programmer> cobolNotAdult = programmers.stream()
                .filter((p) -> {
                    return p.getPerson().getAge()<18 & p.getLanguages().contains("Cobol");
        }).collect(Collectors.toList());
        System.out.println(cobolNotAdult);
        List<Programmer> moreThanOne = programmers.stream()
                .filter(p ->p.getLanguages().size()>1)
                .collect(Collectors.toList());
        System.out.println(moreThanOne);
        List<Programmer> femaleJCpp = programmers.stream()
                .filter(p -> !p.getPerson().isMale() & p.getLanguages().contains("Java") & p.getLanguages().contains("Cpp"))
                .collect(Collectors.toList());
        System.out.println(femaleJCpp);
        List<String> name = programmers.stream()
...
//        najstarsza osoba
        ...
        List<Person> personList = programmers.stream()
                .map(programmer -> programmer.getPerson())
                .sorted(personComparator).collect(Collectors.toList());
        Optional<Person> najstarsza = personList.stream().findFirst();
        System.out.println(najstarsza);
//  *******************************************
        ...
        List<String> nazwiskaWiecejNizJeden = programmers.stream()
                .filter(programmer -> programmer.getLanguages().size()>1)
                .map(programmer -> programmer.getPerson().getLastName())
                .collect(Collectors.toList());
        Boolean osobaBezJezyka = programmers.stream()
                .anyMatch(programmer -> programmer.getLanguages().isEmpty());
        System.out.println("Istnieje osoba bez języka: " + osobaBezJezyka);


        int jezykiKobiety = programmers.stream()
                .filter(programmer -> !programmer.getPerson().isMale())
                .flatMap(programmer -> programmer.getLanguages().stream())
                .collect(Collectors.toSet())
                .size();
        System.out.println("Liczba języków opanowanych przez programistki to: " + jezykiKobiety);
    }
}
